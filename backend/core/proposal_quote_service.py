"""Read-only Deriv digit proposal pricing for DSNPFX research and demo execution.

This service uses the public Options WebSocket only. It never authenticates and
never buys. V12 can request an exact-stake fresh proposal here, then hand the
proposal ID to the separately authenticated demo-only execution controller.
"""
...
def _cache_key(symbol, contract_type, digit):
...
def get_cached_match_quote(symbol, digit):
    return get_cached_quote(symbol, "DIGITMATCH", digit)


def get_cached_differ_quote(symbol, digit):
    return get_cached_quote(symbol, "DIGITDIFF", digit)


def _cache_quote(symbol, contract_type, digit, payload):
...
class ProposalQuoteClient:
    ...
    async def request_quote(self, symbol, digit, contract_type, *, amount=None):
        symbol = str(symbol)
        digit = int(digit)
        contract_type = str(contract_type).upper()
        amount = PROPOSAL_STAKE if amount is None else float(amount)
        if not 0 <= digit <= 9:
        ...
        try:
            ...
            while True:
                ...
                break_even = calculate_break_even_probability(ask, payout)
                return {
    ...
    async def request_match_quote(self, symbol, digit, *, amount=None):
        return await self.request_quote(symbol, digit, "DIGITMATCH", amount=amount)

    async def request_differ_quote(self, symbol, digit, *, amount=None):
        return await self.request_quote(symbol, digit, "DIGITDIFF", amount=amount)


async def request_live_match_quote_once(symbol, digit, *, stake=None):
    """Request a fresh exact-barrier, exact-stake MATCH proposal.

    A dedicated short-lived public socket avoids waiting for the background
    cache. Custom-stake quotes are not placed into the research cache because
    the cache key intentionally does not include amount.
    """
    amount = PROPOSAL_STAKE if stake is None else float(stake)
    client = ProposalQuoteClient()
    try:
        quote = await client.request_match_quote(symbol, digit, amount=amount)
        if isinstance(quote, dict) and amount == PROPOSAL_STAKE:
...
def _research_quote_targets(symbol, market):
    ...
    digit = _valid_digit(hot.get("candidate"))
    if digit is not None:
        targets.add((symbol, "DIGITMATCH", digit))
    ...
    for window in (200, 500, 1000):
        ...
        digit = _valid_digit(report.get("candidate"))
        if digit is not None:
            targets.add((symbol, "DIGITMATCH", digit))
    cold20 = metadata.get("cold_20_differs") or {}
    digit = _valid_digit(cold20.get("candidate"))
    if digit is not None:
        targets.add((symbol, "DIGITDIFF", digit))
    return targets
