(() => {
  ...
  function ensureStyles(){
    ...
    const s=document.createElement("style"); s.id="v11-demo-style"; s.textContent=`
      ...
      .v11-baseline{margin-top:14px;padding:12px;border:1px solid rgba(255,120,120,.18);border-radius:12px;background:rgba(255,90,90,.035)}
      .v11-baseline strong{display:block;margin-bottom:5px}.v11-baseline span{font-size:12px;opacity:.75;line-height:1.5}
      .v11-gate{margin:12px 0;padding:11px 12px;border:1px solid rgba(0,220,150,.16);border-radius:12px;background:rgba(0,220,150,.035);font-size:12px;line-height:1.5}
  ...
  function ensurePanel(){
    ...
    panel.innerHTML=`<div class="panel-heading"><div><p class="eyebrow">V11.1 Selective Trader</p><h2>Strict Demo / Paper P&L</h2></div><span class="watch-badge">LIVE BUY OFF</span></div>
    <div class="v11-gate"><strong>Strict gate:</strong> no paper trade unless independent validation has at least 100 resolved outcomes and its 95% lower confidence bound is above the recorded live DIGITMATCH break-even. Eligible conditions: <b data-v11="eligible">0</b>.</div>
    ...
    <div class="v11-table-wrap"><table class="v11-table"><thead><tr><th>Time</th><th>Market</th><th>Model</th><th>Condition</th><th>Digit</th><th>Result</th><th>P&L</th><th>Break-even</th></tr></thead><tbody data-v11="body"><tr><td colspan="8">Waiting for an independently validated edge...</td></tr></tbody></table></div>
    <div class="v11-baseline" data-v11="baseline"><strong>V11.0 failed baseline preserved</strong><span>Loading historical results...</span></div>
    <p class="v11-note">V11.1 does not buy contracts. The failed permissive V11.0 ledger remains visible as evidence; it is not erased or mixed into the stricter strategy. Real-money execution remains disabled.</p>`;
    evidence.prepend(panel); return panel;
  }
  function render(data){
    const p=ensurePanel(); if(!p) return;
    p.querySelector('[data-v11="eligible"]').textContent=String(data.eligible_conditions||0);
    ...
    p.querySelector('[data-v11="body"]').innerHTML=rows.length?rows.map(r=>`<tr><td>${esc(r.created_at||"")}</td><td>${esc(r.symbol)}</td><td>${esc(r.model)}</td><td>${esc(`${r.condition_family}: ${r.condition_value}`)}</td><td>${esc(r.prediction)}</td><td>${esc(r.result||"PENDING")}</td><td>${r.pnl_units==null?"--":num(r.pnl_units,4)}</td><td>${num(r.break_even_probability_pct)}%</td></tr>`).join(""):'<tr><td colspan="8">No V11.1 trades yet. Waiting for a condition whose validation lower-95% clears live break-even.</td></tr>';
    const b=data.failed_baseline||{};
    p.querySelector('[data-v11="baseline"]').innerHTML=`<strong>V11.0 failed baseline preserved</strong><span>${esc(b.resolved||0)} resolved · ${esc(b.wins||0)} wins / ${esc(b.losses||0)} losses · ${num(b.accuracy_pct)}% accuracy · ${num(b.pnl_units,4)} P&L units · ${num(b.roi_pct_on_unit_stakes)}% ROI · max loss streak ${esc(b.max_consecutive_losses||0)}. This strategy is rejected and will not place new paper trades.</span>`;
  }
  async function refresh(){
    ...
    catch(e){const p=ensurePanel(); if(p) p.querySelector('[data-v11="body"]').innerHTML=`<tr><td colspan="8">V11.1 results unavailable: ${esc(e.message||e)}</td></tr>`;}
  }
